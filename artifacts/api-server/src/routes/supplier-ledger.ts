...
router.post("/:id/ledger", async (req, res): Promise<any> => {
  try {
    ...
    const entry = await db.transaction(async (tx) => {
      // prevent creating manual entries in closed financial periods
      try {
...
router.delete("/:id/ledger/:entryId", async (req, res): Promise<any> => {
  try {
    ...
    // prevent deleting entries that fall in closed periods
    try {
      const months = await import("../services/months.service.js");
...
router.post("/:id/payments", async (req, res): Promise<any> => {
  try {
    ...
    // prevent recording supplier payments into closed periods
    try {
      const months = await import("../services/months.service.js");
      if (await months.isDateInClosedPeriod(paymentDate)) {
        throw new months.MonthClosedError(paymentDate);
      }
    } catch (err) {
      if (err && (err as Error).name === "MonthClosedError") throw err;
    }
