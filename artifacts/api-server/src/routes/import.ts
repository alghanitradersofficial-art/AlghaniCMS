...
// ─── STAFF ATTENDANCE IMPORT (backdated/historical) ──────────────────────────
// Expected columns: staff_id (or staff_name), date (YYYY-MM-DD), status
// (present/absent/half_day/leave), note. Upserts per staff_id+date, same as
// the single-day mark endpoint in routes/staff.ts.
router.post("/staff-attendance", upload.single("file"), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: "No file uploaded" });
    const wb = new ExcelJS.Workbook();
    const ext = path.extname(req.file.originalname).toLowerCase();
    ...
    const nameIdx = Math.max(headers.indexOf("staff_name"), headers.indexOf("name"));
    const dateIdx = headers.indexOf("date");
    const statusIdx = headers.indexOf("status");
    const noteIdx = headers.indexOf("note");

    if (dateIdx < 0 || statusIdx < 0 || (idIdx < 0 && nameIdx < 0)) {
      return res.status(400).json({ error: "Expected columns: staff_id (or staff_name), date, status" });
    }

    const rowsToProcess: Array<{ rn: number; cells: unknown[] }> = [];
    ws.eachRow((row, rn) => {
      if (rn === 1) return;
      const cells: unknown[] = [];
      row.eachCell({ includeEmpty: true }, (c) => cells.push(c.value));
      rowsToProcess.push({ rn, cells });
    });

    let imported = 0;
    const errors: string[] = [];
    for (const { rn, cells } of rowsToProcess) {
      try {
        let staffId: number | null = idIdx >= 0 ? parseInt(parseStr(cells[idIdx])) || null : null;
        if (!staffId && nameIdx >= 0) {
          const name = parseStr(cells[nameIdx]);
          const found = await pool.query(`SELECT id FROM staff WHERE name ILIKE $1 LIMIT 1`, [name]);
          staffId = found.rows[0]?.id ?? null;
        }
        if (!staffId) { errors.push(`Row ${rn}: staff not found`); continue; }

        const date = parseStr(cells[dateIdx]);
        const status = parseStr(cells[statusIdx]).toLowerCase().replace(/\s+/g, "_");
        if (!["present", "absent", "half_day", "leave"].includes(status)) {
          errors.push(`Row ${rn}: invalid status "${status}"`);
          continue;
        }
        const note = noteIdx >= 0 ? parseStr(cells[noteIdx]) || null : null;

        await pool.query(
          `INSERT INTO staff_attendance (staff_id, date, status, note) VALUES ($1,$2,$3,$4)
           ON CONFLICT (staff_id, date) DO UPDATE SET status = $3, note = $4`,
          [staffId, date, status, note],
        );
        imported++;
      } catch (e: unknown) {
        errors.push(`Row ${rn}: ${(e as Error).message}`);
      }
    }

    return res.json({ imported, total: rowsToProcess.length, errors: errors.slice(0, 20), message: `Imported ${imported} attendance records` });
  } catch (error) {
    return res.status(500).json({ error: "Import failed: " + (error as Error).message });
  }
});
