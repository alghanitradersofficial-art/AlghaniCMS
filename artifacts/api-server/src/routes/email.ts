...
import { buildFullReportWorkbook } from "./export.js";
...
`;
    ...
    const toList = recipients?.length ? recipients : (process.env.CEO_EMAIL ? [process.env.CEO_EMAIL] : []);
    if (!toList.length) {
      return res.status(400).json({ success: false, message: "No recipients configured. Set CEO_EMAIL env var or provide recipients in request." });
    }

    // Build report workbook and decide whether to attach full or summary
    const rangeQuery = { preset: reportType, from: req.body.from, to: req.body.to };
    let wb: ExcelJS.Workbook;
    try {
      wb = await buildFullReportWorkbook(rangeQuery as any);
    } catch (err) {
      console.error("Email report workbook generation failed:", (err as any)?.stack || err);
      return res.status(500).json({ error: "Failed to build report workbook", detail: (err as any)?.message || String(err) });
    }
    const buf = await wb.xlsx.writeBuffer();
    const maxBytes = Number(process.env.REPORT_ATTACHMENT_MAX_BYTES || 5 * 1024 * 1024);
    const attachFullRequested = Boolean(req.body?.attachFull);
    const attachments = [] as any[];
    if (buf.byteLength <= maxBytes || attachFullRequested) {
      if (attachFullRequested && buf.byteLength > maxBytes) {
        console.warn(`attachFull requested but report size ${buf.byteLength} exceeds REPORT_ATTACHMENT_MAX_BYTES=${maxBytes}; attaching large file by request`);
      }
      attachments.push({ filename: "full-report.xlsx", content: Buffer.from(buf) });
    } else {
      // fallback: attach small summary workbook
      const summaryWb = new ExcelJS.Workbook();
      const ws = summaryWb.addWorksheet("Summary");
      ws.columns = [{ key: "k", width: 40 }, { key: "v", width: 30 }];
      ws.addRow(["Report Type", reportType]);
      ws.addRow(["Generated At", new Date().toLocaleString("en-PK")]);
      ws.addRow(["Sales Revenue", `Rs. ${salesTotal.toLocaleString()}`]);
      ws.addRow(["Purchases", `Rs. ${purchTotal.toLocaleString()}`]);
      ws.addRow(["Expenses", `Rs. ${expTotal.toLocaleString()}`]);
      ws.addRow(["Net Profit", `Rs. ${(salesTotal - purchTotal - expTotal).toLocaleString()}`]);
      ws.addRow(["Low Stock", String(lowStock)]);
      const smallBuf = await summaryWb.xlsx.writeBuffer();
      attachments.push({ filename: "report-summary.xlsx", content: Buffer.from(smallBuf) });
    }
