...
export async function appendGeneralLedgerEntry(
  ...
  try {
    ...
    if (await months.isDateInClosedPeriod(params.date)) {
      throw new months.MonthClosedError(params.date);
    }
  } catch (err) {
    if (err && (err as Error).name === "MonthClosedError") throw err;
    // if import failed for unexpected reason, continue and let DB handle errors
  }
