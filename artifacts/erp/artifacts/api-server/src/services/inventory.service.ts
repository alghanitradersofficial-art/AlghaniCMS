...
export async function adjustStock(params: { productId: number; direction: "increase" | "decrease"; quantity: number; reason: string; notes?: string | null; performedByUserId?: number | null; date?: Date }) {
  const { productId, direction, quantity, reason, notes, performedByUserId, date } = params;
  ...
  const [row] = await db.transaction(async (tx) => {
    if (date && await import("./months.service.js").then(m=>m.isDateInClosedPeriod(date))) {
      throw (await import("./months.service.js")).MonthClosedError ? new (await import("./months.service.js")).MonthClosedError(date) : new Error("Financial period closed");
    }
