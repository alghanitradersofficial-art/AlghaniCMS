...
import { Menu, X, Moon, SunMedium, LayoutDashboard, BarChart2, Package, ShoppingCart, Wallet, ReceiptText, MoreHorizontal, Plus, ChevronRight, PanelLeftClose, PanelLeftOpen, Truck, Users, Settings2, BookText, LogOut } from "lucide-react";
...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="h-screen overflow-hidden bg-transparent text-foreground transition-all duration-300 ease-in-out">
      <div className="mx-auto flex h-full w-full flex-col lg:max-w-7xl lg:flex-row">
        {showDesktopSidebar && (
          <aside className={`${collapsed ? "w-20" : "w-72 xl:w-80"} hidden h-screen flex-col sticky top-0 border-r border-border/70 bg-card/90 shadow-inner backdrop-blur-xl lg:flex overflow-hidden`}>
              ...
              <div className="flex items-center justify-between px-4 py-4 border-b border-border/60">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex h-10 items-center gap-2 rounded-2xl border border-destructive/60 bg-destructive/5 px-3 text-sm font-semibold text-destructive transition hover:bg-destructive/10"
                    aria-label="Logout"
                  >
        ...
        <main className="flex min-w-0 flex-1 flex-col pb-28 lg:pb-6 overflow-y-auto">
              ...
              <div className="ml-auto flex items-center gap-3">
                ...
                {isMobileView && (
                  <button
                    ...
                    className="lg:hidden inline-flex h-10 w-10 items-center justify-center rounded-full border border-border/60 text-muted-foreground transition hover:bg-primary/10 hover:text-primary"
                    aria-label="Logout"
                  >
                    <LogOut className="h-4 w-4" />
                  </button>
